...
import 'package:markd/markdown.dart' as m;
...
class _PreviewPageState extends State<PreviewPage> {
  ...
  _processContent() async {
    ...
    content = content.replaceAllMapped(RegExp(r'(?<=\]\(@note\/).*(?=\))'), (match) {
...
<head>
''' +
        (Provider.of<ThemeNotifier>(context, listen: false).currentTheme == ThemeType.light
            ? ''
            : '''
  <style>
  body {
...
/*     generatedPreview = generatedPreview.replaceAll('\\ ', ' '); */
    generatedPreview = generatedPreview
        .replaceAll(
            'src="@attachment/', 'src="' + 'file://' + PrefService.getString('notable_attachments_directory') + '/')
        .replaceAll('src="/', 'src="' + 'file://' + PrefService.getString('notable_notes_directory') + '/');

    generatedPreview = generatedPreview.replaceAll('<img ', '<img width="100%" ');

    // print(generatedPreview.split('<body>')[1]);

    int checkboxIndex = -1;

    generatedPreview = generatedPreview.replaceAllMapped('disabled="disabled" class="todo" type="checkbox"', (match) {
      checkboxIndex++;
...
Page resource error:
  ...
  isForMainFrame: ${error.isForMainFrame}
      ...
      ..addJavaScriptChannel('notelesscheckbox', onMessageReceived: (msg) async {
  ...
  Widget build(BuildContext context) {
    ...
    _controller?.runJavaScript("function toMobile(){"
  ...
  void _navigateToNote(String title) async {
    ...
    Note newNote = await PersistentStore.readNote(File('${PrefService.getString('notable_notes_directory')}/${title}'));
    if (newNote == null) {
      // TODO Show Error
    } else {
      Navigator.of(context).push(MaterialPageRoute(builder: (context) => EditPage(newNote, widget.store)));
    }
  }
