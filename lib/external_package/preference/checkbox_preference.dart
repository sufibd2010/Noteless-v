...
class _CheckboxPreferenceState extends State<CheckboxPreference> {
  ...
  Widget build(BuildContext context) {
    return ListTile(
      ...
      trailing: Checkbox(
        value: PrefService.getBool(widget.localKey!) !=null ? PrefService.getBool(widget.localKey!) : widget.defaultVal,
        onChanged:
            widget.disabled! ? null : (val) => val! ? onEnable() : onDisable(),
      ),
      onTap: (widget.ignoreTileTap! || widget.disabled!)
          ? null
          : () => (PrefService.getBool(widget.localKey!) !=null ? PrefService.getBool(widget.localKey!) : widget.defaultVal!)
              ? onDisable()
              : onEnable(),
    );
  }
